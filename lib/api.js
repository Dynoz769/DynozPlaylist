...
async function route(request, ctx) {
  ...
  if (path === '/api/search' && method === 'GET') {
    // Had carian setiap peranti supaya kuota YouTube tak habis bila app terbuka kepada umum
    const wait = (await storage.rateLimited?.(`search:${ctx.ip || 'unknown'}`, 40, 60 * 60 * 1000)) ?? 0;
    if (wait) {
      return json(429, { ok: false, error: `Terlalu banyak carian dari peranti ni. Cuba lagi dalam ${Math.ceil(wait / 60)} minit.` });
    }
    try {
